...
class CrewCertificate {
  ...
  #setAllValuesFromDigitalSeal() {
    ...
    if (sealMRZ[21] !== TravelDocument.generateMRZCheckDigit(
      sealMRZ.slice(15, 21).replace(/ /gi, "<")
    )) {
      throw new EvalError(
        `Date of birth check digit '${sealMRZ[21]}' does not match for date` +
            `of birth '${sealMRZ.slice(15, 21).replace(/ /gi, "<")}'.`
      );
    }
    ...
    if (parseInt(yearOfBirth, 10) >= twoDigitYearStart) {
      this.#document.birthDate = `19${yearOfBirth}-${monthOfBirth}-` +
          `${dayOfBirth}`;
    } else {
      this.#document.birthDate = `20${yearOfBirth}-${monthOfBirth}-` +
          `${dayOfBirth}`;
    }
    ...
    this.#document.expirationDate = `20${yearExpiration}-${monthExpiration}` +
        `-${dayExpiration}`;
