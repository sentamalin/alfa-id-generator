 ...
 */
export class CrewCertificateRenderer {
  /**
  ...
  async generateCardBack(model, fallback) {
    ...
    ctx.fillStyle = this.textColor;
    ctx.font = CrewCertificateRenderer.#dataFont;
    model.declaration.toUpperCase().split(/\r?\n/).forEach((line, i) => {
      ctx.fillText(
        line,
        CrewCertificateRenderer.#backColumns,
    ...
    ctx.fillText(
      `${dateToVIZ(model.issueDate)}—${model.placeOfIssue.toUpperCase()}`,
      CrewCertificateRenderer.#backColumns,
